...
describe("PostgresStore supervisor checkpoint settlement", () => {
  it("persists and verifies one terminal for each frozen direction", async () => {
    ...
    ).resolves.toBe("00000000-0000-4000-8000-000000000031");

    expect(fake.calls).toHaveLength(4);
    expect(fake.calls[1]?.values).toContain(SOURCE);
    expect(fake.calls[1]?.values).toContain(DESTINATION);
    ...
    expect(fake.calls[1]?.text).toContain("COALESCE(previous.accepted_input_sequence+1,0)");
  });
