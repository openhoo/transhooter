...
function ArchiveHeader({ status }: { status: string }) {
  ...
  return (
    <header className="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Recorded consultation</p>
        <h1 className="mt-1 font-serif text-2xl font-semibold tracking-tight text-foreground md:text-3xl">Archive evidence</h1>
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Reconciled recordings, captions, provider exchanges, and the signed inventory for this consultation.
        </p>
      </div>
      <span className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-semibold ${complete ? "bg-verified text-verified-foreground" : "bg-attention text-attention-foreground"}`}>
        {status}
      </span>
    </header>
  );
...
function ArchiveFinalizingNotice({ status }: { status: string }) {
  ...
  return (
    <section className="rounded-md border border-attention/70 bg-attention/30 p-4" aria-labelledby="archive-finalizing-title">
      <h2 className="font-serif text-base font-semibold" id="archive-finalizing-title">Finalizing the archive</h2>
      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
        Original recordings and provider evidence are still being reconciled. This page will show verified versions and checksums when available.
      </p>
      <div className="mt-3"><RefreshArchiveStatus /></div>
    </section>
  );
}
...
function ActiveLegalHolds({ holds }: { holds: ArchiveHold[] }) {
  ...
  return (
    <section className="rounded-md border border-attention/70 bg-attention/30 p-4" aria-labelledby="active-holds-title">
      <h2 className="font-serif text-base font-semibold" id="active-holds-title">Active legal holds</h2>
      <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
        {holds.map((hold) => <li key={hold.id}>{hold.reason} <span className="font-mono text-xs">({hold.id})</span></li>)}
      </ul>
    </section>
  );
}
...
function ArchiveProof({ archive }: { archive: ArchiveView }) {
  const hasProof = archive.inventoryVersion || archive.inventorySha256 || archive.egressIds.length > 0 || archive.providerAttemptIds.length > 0;
  ...
  return (
    <section className="rounded-md border border-border bg-card" aria-labelledby="archive-proof-title">
      <div className="border-b border-border px-5 py-4">
        <h2 className="font-serif text-lg font-semibold" id="archive-proof-title">Archive proof</h2>
        <p className="mt-1 text-sm text-muted-foreground">Signed inventory and persisted processing identifiers.</p>
      </div>
...
function InventoryGaps({ gaps }: { gaps: ArchiveGap[] }) {
  ...
  return (
    <section className="rounded-md border border-destructive/40 bg-destructive/5 p-5" aria-labelledby="inventory-gaps-title">
      <h2 className="font-serif text-lg font-semibold text-destructive" id="inventory-gaps-title">Inventory gaps</h2>
      <ul className="mt-3 space-y-2 text-sm leading-relaxed text-muted-foreground">
        {gaps.map((gap, index) => <li key={`${gap.class}:${String(index)}`}><strong className="text-foreground">{gap.class}</strong> — {gap.detail}</li>)}
      </ul>
    </section>
  );
}
...
function ArchiveObjectSection({ archiveId, label, objects }: { archiveId: string; label: string; objects: ArchiveObject[] }) {
  const artifactCount = `${String(objects.length)} ${objects.length === 1 ? "artifact" : "artifacts"} on this page`;

  return (
    <details className="group overflow-hidden rounded-md border border-border bg-card" open={objects.length > 0}>
      ...
      {objects.length === 0 ? (
        <p className="border-t border-border px-5 py-4 text-sm text-muted-foreground">No artifacts in this group on this page.</p>
      ) : (
        <div className="overflow-x-auto border-t border-border">
          <table className="archiveObjectTable w-full text-sm">
            <caption className="srOnly">{label}, {artifactCount}</caption>
            <thead className="bg-secondary"><tr><th>Artifact</th><th>Type / size</th><th>Version / SHA-256</th><th><span className="srOnly">Action</span></th></tr></thead>
            <tbody>{objects.map((object) => <ArchiveObjectRow archiveId={archiveId} object={object} key={object.id} />)}</tbody>
          </table>
        </div>
      )}
    </details>
...
export default async function ArchivePage({ params, searchParams }: ArchivePageProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ArchivePagination archiveId={id} currentCursor={currentCursor} nextCursor={archive.nextCursor} previousCursor={previousCursor} />
      {archive.canAdminister && (
        <ArchiveAdminActions archiveId={archive.id} consultationId={archive.consultationId} activeHolds={archive.activeHolds} />
      )}
    </div>
  );
}
