    ...
    <div className="flex flex-col gap-6">
      <header className="border-b border-border pb-5">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Recorded incidents from live sessions and archive processing. Every item preserves the
          evidence required for supervisory review; provider errors never silently switch vendors.
        </p>
      </header>
            ...
            <li key={failure.id}>
              <article className="overflow-hidden rounded-md border border-border bg-card">
                <header className="flex flex-col gap-3 border-b border-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="font-mono text-sm font-semibold text-destructive">
                      {failure.code}
                    </h2>
                    <p className="mt-1 text-xs text-muted-foreground">{failure.stage}</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <code className="rounded-full border border-border px-2 py-1 text-xs">
                      {failure.consultationId}
                    </code>
                    <time className="text-xs text-muted-foreground" dateTime={failure.occurredAt}>
                ...
                <div className="px-5 py-4">
                  ...
                  <pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-md bg-secondary p-4 font-mono text-xs leading-relaxed text-foreground">
                    {formatEvidence(failure.summary)}
                  </pre>
                  {failure.archiveId && (
                    <Link
