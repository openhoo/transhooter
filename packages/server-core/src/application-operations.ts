...
export class DrizzleApplicationOperations implements ApplicationOperations {
  ...
  async checkpoint(input: CheckpointInput): Promise<boolean> {
    ...
    const result = await this.database.execute(
      sql`INSERT INTO worker_checkpoints(id,consultation_id,generation,worker_id,worker_epoch,write_epoch,high_watermark,previous_hash,checkpoint_hash,expected_ids,observed_ids,gaps,terminal,object_key,created_at) SELECT ${checkpoint.checkpointId},${input.consultationId},${input.generation},${input.workerId},${checkpoint.workerEpoch},${input.writeEpoch},${checkpoint.occurredAtMs},${checkpoint.previousCheckpointSha256},${checkpoint.highWatermarkSha256},${expected}::jsonb,${observed}::jsonb,${gaps}::jsonb,${checkpoint.terminal},${input.objectKey},to_timestamp(${checkpoint.occurredAtMs}/1000.0) FROM worker_reservations r JOIN archives a ON a.consultation_id=r.consultation_id WHERE r.consultation_id=${input.consultationId} AND r.generation=${input.generation} AND r.worker_id=${input.workerId} AND r.epoch=${checkpoint.workerEpoch} AND r.fenced_at IS NULL AND r.released_at IS NULL AND a.write_epoch=${input.writeEpoch} ON CONFLICT (id) DO NOTHING RETURNING id`,
    );

    if (result.rowCount !== 1) {
      const replay = await this.database.execute(
        sql`SELECT 1 FROM worker_checkpoints WHERE id=${checkpoint.checkpointId} AND consultation_id=${input.consultationId} AND generation=${input.generation} AND worker_id=${input.workerId} AND worker_epoch=${checkpoint.workerEpoch} AND write_epoch=${input.writeEpoch} AND high_watermark=${checkpoint.occurredAtMs} AND previous_hash IS NOT DISTINCT FROM ${checkpoint.previousCheckpointSha256} AND checkpoint_hash=${checkpoint.highWatermarkSha256} AND expected_ids=${expected}::jsonb AND observed_ids=${observed}::jsonb AND gaps=${gaps}::jsonb AND terminal=${checkpoint.terminal} AND object_key=${input.objectKey}`,
      );
    ...
    if (checkpoint.terminal) {
      const terminal = await this.database.execute(
        sql`WITH terminal_epoch AS (
          UPDATE worker_job_epochs
          SET terminal_checkpoint_id=COALESCE(terminal_checkpoint_id,${checkpoint.checkpointId}),
            terminal_outcome=COALESCE(terminal_outcome,'clean'),
            terminal_at=COALESCE(terminal_at,to_timestamp(${checkpoint.occurredAtMs}/1000.0))
          WHERE consultation_id=${input.consultationId} AND generation=${input.generation}
            AND worker_id=${input.workerId} AND epoch=${checkpoint.workerEpoch}
            AND (terminal_checkpoint_id IS NULL OR terminal_checkpoint_id=${checkpoint.checkpointId})
        ...
        RETURNING reservation.worker_id`,
      );
