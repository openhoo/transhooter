...
import pytest
from transhooter_spool import EncryptedSpool, SampleRange, deterministic_roomy_capacity
...
def authority(store: EncryptedSpool, job: JobMetadata) -> object:
    return store.open_consultation_producer(
        meeting_id=job.consultation_id,
        generation=job.generation,
        worker_id=job.worker_identity,
        worker_epoch=job.worker_epoch,
        write_epoch=job.write_epoch,
    )
...
async def test_provider_reports_commit_stage_and_terminal_evidence_exactly_once(
    ...
    producer = authority(encrypted_spool, job)
    ...
    assert all(
        encrypted_spool.context(reference.object_id).stage == "terminal"
    ...
    for attempt_stage, terminal in terminals:
        ...
        context = encrypted_spool.context(evidence.object_id)
        assert context.attempt_id == attempt_id
        assert context.stage == attempt_stage
