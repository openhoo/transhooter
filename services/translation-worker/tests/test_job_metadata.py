...
from transhooter_spool import SpoolUnavailable
...
async def test_runtime_supervision_stops_gate_users_before_quota_cleanup(
    ...
    async def drain_runtime() -> dict[UUID, object]:
        stream_task.cancel()
...
@pytest.mark.asyncio
async def test_initialization_failure_cleanup_is_memoized_and_preserves_failures() -> None:
    events: list[str] = []
...
@pytest.mark.asyncio
async def test_initialization_cleanup_preserves_unpublish_setup_failure() -> None:
    events: list[str] = []
...
@pytest.mark.asyncio
async def test_early_consultation_failure_releases_spool_and_producer_authority(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    configure_contract_schema(monkeypatch)
    events: list[str] = []
    ...
    class Spool:
        def open_consultation_producer(self, **_kwargs: object) -> Authority:
            events.append("authority.open")
            return Authority()

        def close(self) -> None:
            events.append("spool.close")

    async def fail_before_runtime_cleanup(*_args: object) -> None:
        raise RuntimeError("invalid frozen directions")

    monkeypatch.setattr("transhooter_worker.runtime.job._spool", Spool)
    monkeypatch.setattr(
        "transhooter_worker.runtime.job._run_owned_consultation",
        fail_before_runtime_cleanup,
    )
    ctx = SimpleNamespace(job=SimpleNamespace(metadata=json.dumps(job_metadata_payload())))

    with pytest.raises(RuntimeError, match="invalid frozen directions"):
        await _run_consultation(ctx)  # type: ignore[arg-type]

    assert events == ["authority.open", "authority.close", "spool.close"]


@pytest.mark.asyncio
async def test_provider_preflight_failure_propagates_without_failure_delivery() -> None:
    ...
    with pytest.raises(RuntimeError, match="capability mismatch"):
        await _reported_preflight(fail_preflight)


def test_worker_shutdown_timeout_is_bounded_for_preservation_restart() -> None:
    options = _worker_options("api-key", "api-secret")

    assert options.shutdown_process_timeout == 30.0

    assert options.job_executor_type.value == "thread"


def test_spool_failure_exits_job_with_fatal_preservation_code(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    exits: list[int] = []
...
@pytest.mark.asyncio
async def test_spool_failure_requests_fatal_job_exit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    exits: list[int] = []
...
@pytest.mark.asyncio
async def test_nested_spool_failure_requests_fatal_job_exit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    exits: list[int] = []
