...
import httpx
import pytest
from transhooter_spool import RawRef
...
def test_google_voice_selection_rejects_missing_wrong_locale_and_unapproved_request(
    ...
    with pytest.raises(RuntimeError, match=r"capability-approved voice|no approved voice"):
...
def test_effective_voice_is_identical_in_journal_and_provider_config(
    ...
    metadata = dict(_probe_metadata("google-eu", "en-US", "de-DE", voice))
    ...
    assert execution_profile.probe_voice == voice
    assert execution_profile.probe_voice_locale == "de-DE"
    assert providers.tts._config.probe_voice == metadata["voice"]
