...
def test_service_version_resolves_worker_distribution(monkeypatch: pytest.MonkeyPatch) -> None:
    requested: list[str] = []
    ...
    monkeypatch.setattr(telemetry, "version", version)
    assert telemetry._service_version() == "9.8.7"
    assert requested == ["transhooter-translation-worker"]
