    ...
    async def finish(self) -> DirectionResult:
        async with self._finish_lock:
            if self._state is Lifecycle.TERMINAL:
                return DirectionResult(self._last_input, self._last_output)
            ...
            direction_result = DirectionResult(self._last_input, self._last_output)
            if shutdown_error is not None:
                raise shutdown_error
            return direction_result

    async def cancel(self) -> DirectionResult:
