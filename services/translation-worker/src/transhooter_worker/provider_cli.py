...
from transhooter_worker.adapters.diagnostic_archive import (
    DiagnosticS3Archive,
...
async def _publish_capabilities(payload: dict[str, object]) -> None:
    ...
    token = (await asyncio.to_thread(Path(token_file).read_text, "utf-8")).strip()
    if not token:
...
def _probe_metadata(
    profile_name: str, source: str, target: str, voice: str | None
) -> tuple[tuple[str, str], ...]:
    return (
...
def _probe_archive() -> DiagnosticS3Archive:
    return build_diagnostic_archive(Path(_required("SPOOL_PATH")))
...
def _archive_probe_evidence(
    ...
    for reference in evidence:
        context = journal.context(reference.object_id)
        ...
        record = archive.put_create_once(
            (
                f"v1/meetings/{meeting_id}/pipeline/{context.stage}/probe/"
                f"{context.attempt_id}/{context.ordinal:020d}.bin"
            ),
            body,
            context.media_type,
...
async def _one_probe(
    ...
    journal = _spool()
    authority = journal.open_consultation_producer(
        meeting_id=meeting_id,
        generation=1,
        worker_id=meeting_id,
        worker_epoch=1,
        write_epoch=0,
    )
    scoped_journal = ScopedExchangeJournal(
        journal,
        authority,
        (
            ("generation", "1"),
            ("workerId", str(meeting_id)),
            ("workerEpoch", "1"),
            ("writeEpoch", "0"),
            *_probe_metadata(profile_name, source, target, None),
        ),
    )
    before_ids = {ref.object_id for ref, _ in journal.committed()}
    async with _provider_lifecycle(
        ProviderRegistry.construct(profile, meeting_id, scoped_journal)
    ...
    scoped_journal = ScopedExchangeJournal(
        ...
        (
            ...
            ("workerEpoch", "1"),
            ("writeEpoch", "0"),
            *_probe_metadata(profile_name, source, target, effective_voice),
        ),
    )
    async with _provider_lifecycle(
        ProviderRegistry.construct(profile, meeting_id, scoped_journal)
    ) as providers:
        probe_input = await asyncio.to_thread(audio.read_bytes)
        journal.append(
            authority,
            meeting_id=meeting_id,
...
async def _run_refresh_or_preflight(args: argparse.Namespace) -> dict[str, object]:
    ...
    scoped_journal = ScopedExchangeJournal(
        journal,
        authority,
        (("generation", "1"), ("workerId", str(run_id)), ("workerEpoch", "1"), ("writeEpoch", "0")),
    )
    async with _provider_lifecycle(
        ProviderRegistry.construct(profile, run_id, scoped_journal)
    ) as bootstrap_providers:
        bootstrap_stages = (
