...
from tests.fakes.s3 import RecordingS3Client
from transhooter_spool_drainer.s3_archive import ArchiveConflict, S3Archive
...
def test_single_part_supplies_precomputed_crc64nvme_without_sdk_calculation(
    tmp_path: Path,
) -> None:
    archive, _ = archive_client(tmp_path)
    body = b"payload"

    args = archive._object_arguments(
        key=f"v1/meetings/{uuid4()}/pipeline/translation/raw/source/0.json",
        content_type="application/json",
        sha256=sha256_hex(body),
        body=body,
    )

    assert args["ChecksumCRC64NVME"] == S3Archive._crc64nvme(body)
    assert "ChecksumAlgorithm" not in args
